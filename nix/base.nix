let
  f =
    ...
    let
      ...
      pseudo-inverse-category-overlay = self: super: {
      ...
      pkgs = import
        ...
        overlays = [
          shpadoinkle-overlay
          pseudo-inverse-category-overlay
        ];
      };
      ...
      pseudo-inverse-category = pkgs.haskell.packages.${compilerjs}.callCabal2nix "pseudo-inverse-category" (gitignore ignorance ../.) {};


    in with pkgs; with lib;

      { build =
          (if isJS && optimize then doCannibalize else x: x) (chill pseudo-inverse-category);

        shell =
          pkgs.haskell.packages.${compilerjs}.shellFor {
            inherit withHoogle;
            packages    = _: [ pseudo-inverse-category ];
            COMPILER    = compilerjs;
